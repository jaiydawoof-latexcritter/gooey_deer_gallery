...
// driveUrl comes from config.json via loader.js (window.SITE_CONFIG)
// Falls back to hardcoded value if config hasn't loaded yet.
...
function ratingVisible(rating) {
  if (!nsfwUnlocked && (rating==='NSFW'||rating==='Suggestive')) return false;
  return true;
}
...
async function init() {
  // Show loading state
  document.getElementById('gallery-grid').innerHTML = '<div style="padding:60px;text-align:center;color:var(--muted)">Loading gallery from Google Drive...</div>';

  // Fetch gallery and refs in parallel
  ...
  renderGallery(); renderRefs(); updateStats(); populateSidebar();
}
...
function renderGallery() {
  ...
  visibleItems.forEach((item,idx) => {
    const restricted = !nsfwUnlocked && (item.rating==='NSFW'||item.rating==='Suggestive');
    const el = document.createElement('div');
    el.className = `gallery-item${restricted?'':' nsfw-unlocked'}`;
    el.style.animationDelay = `${idx*0.04}s`;
    el.innerHTML = `
      <div class="${restricted?'nsfw-blur':''}"><img src="${item.src}" alt="${item.title}" loading="lazy"></div>
      ...
      <div class="item-info">
        ...
        <div><span class="item-type">${item.type}</span><span class="item-badge ${badgeClass(item.rating)}">${item.rating}</span></div>
      </div>`;
...
function renderRefs() {
  ...
  filtered.forEach((ref,idx) => {
    const restricted = !nsfwUnlocked && (ref.rating==='NSFW'||ref.rating==='Suggestive');
    const el = document.createElement('div');
    el.className = `ref-card${restricted?'':' nsfw-unlocked'}`;
    el.style.animationDelay = `${idx*0.06}s`;
    el.innerHTML = `
      <div class="ref-img-wrap${restricted?' nsfw-blur':''}"><img src="${ref.src}" alt="${ref.title}" loading="lazy"></div>
      ${restricted?`<div class="ref-nsfw-cover"><span>${ref.rating.toUpperCase()}</span><p>Enable NSFW to view</p></div>`:''}
      <div class="ref-info">
        <div><div class="ref-title">${ref.title}</div><div class="ref-artist">by ${ref.artist}</div></div>
...
function populateSidebar() {
  ...
  // Filter the pool by current type filter so only relevant artists show
  ...
  // If current artist filter no longer exists in new pool, reset it
  ...
  // Re-mark All Artists as active if needed
...
function filterGalleryType(type,btn) {
  ...
  document.querySelectorAll('#page-gallery .filter-btn').forEach(b=>{
    if(['All','Gooey','Pooltoy','Normal','Fursuit','Latex','Other'].includes(b.textContent)) b.classList.remove('active');
  });
  btn.classList.add('active'); populateSidebar(); renderGallery();
}
function filterGalleryRating(rating,btn) {
  galleryRatingFilter=rating;
  ['g-r-all','g-r-sfw','g-r-sug','g-r-nsfw'].forEach(id=>document.getElementById(id)?.classList.remove('active'));
  btn.classList.add('active'); renderGallery();
}
function filterRefsRating(rating,btn) {
  refsRatingFilter=rating;
  ['r-r-all','r-r-sfw','r-r-sug','r-r-nsfw'].forEach(id=>document.getElementById(id)?.classList.remove('active'));
  btn.classList.add('active'); renderRefs();
}
function unlockNSFW() {
  ...
  renderGallery(); renderRefs();
}
function dismissGate() { document.getElementById('age-gate').classList.add('hidden'); }
function toggleNSFW() {
...
function showLightboxItem() {
  const item=visibleItems[lightboxIndex]; if(!item)return;
...
function showPage(name) {
  ...
  document.querySelectorAll('.nav-link').forEach(l=>{if(l.textContent===map[name])l.classList.add('active');});
}
...
// Wait for loader.js to finish injecting all HTML parts before running
document.addEventListener('parts-loaded', init);
// Fallback: if script loads after parts-loaded already fired
