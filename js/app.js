  ...
  filtered.forEach((ref, idx) => {
    ...
    el.innerHTML =
      '<div class="ref-img-wrap' + (restricted ? ' nsfw-blur' : '') + '">' +
        '<img ' + imgData + ' alt="' + ref.title + '" loading="lazy">' +
      '</div>' +
...
function filterGalleryRating(rating, btn) {
  ...
  ['g-r-all','g-r-sfw','g-r-nsfw'].forEach(id => document.getElementById(id) && document.getElementById(id).classList.remove('active'));
  btn.classList.add('active');
  renderGallery();
}
...
function filterRefsRating(rating, btn) {
  ...
  ['r-r-all','r-r-sfw','r-r-nsfw'].forEach(id => document.getElementById(id) && document.getElementById(id).classList.remove('active'));
  btn.classList.add('active');
  renderRefs();
}
...
function showPage(name) {
  ...
  var map = { gallery:'Gallery', refs:'Ref Sheets', about:'About', contact:'Contact' };
...
function updateStats() {
  ...
  var artistsEl = document.getElementById('stat-artists');
  if (totalEl)   totalEl.textContent   = allGallery.length;
  if (refsEl)    refsEl.textContent    = allRefs.length;
  if (artistsEl) {
    var artists = new Set([...allGallery.map(i => i.artist), ...allRefs.map(r => r.artist)]);
